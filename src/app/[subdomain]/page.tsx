...
export default async function SubdomainPage({ params }: Props) {
  ...
  if (['api', 'dashboard', 'cold-call', 'callback', 'sign-in', 'sign-up', '_next'].includes(subdomain)) {
    notFound()
  }
  ...
  const html = content?.html || '<p>Website coming soon...</p>'

  // Inject booking widget and analytics
  const enhancedHtml = injectWidgets(html, {
...
`
  ...
  return html.replace('</body>', widgetScript + '</body>')
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const businessName = website.ColdCallLead?.businessName || 'Business'
  const category = website.ColdCallLead?.category?.replace(/_/g, ' ') || ''
  const city = website.ColdCallLead?.city || ''

  return {
    title: website.seoTitle || `${businessName} | ${category} in ${city}`,
    description: website.seoDescription || `${businessName} - Professional ${category} services in ${city}. Quality service, trusted by locals.`,
    keywords: website.seoKeywords?.join(', ') || `${category}, ${city}, ${businessName}`,
  }
}
